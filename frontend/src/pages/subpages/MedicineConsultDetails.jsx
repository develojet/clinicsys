...
const MedicineConsultDetails = ({ visitData, onSave }) => {
    ...
    const handleFinalize = () => {
        ...
        console.log("Saving Medicine Consult:", finalData);
    ...
    return (
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 md:p-10 relative animate-in fade-in slide-in-from-top-4 duration-500">
            {/* Header Badge - Positioned to float over the border */}
            <div className="absolute top-0 left-10 -translate-y-1/2 bg-slate-900 text-white px-5 py-1.5 rounded-full text-[10px] font-black tracking-[0.2em] uppercase shadow-lg z-10">
                Medicine Consultation #{visitData.id} — {visitData.date}
            </div>
                ...
                <div className="space-y-8 animate-in fade-in text-left">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <textarea className="w-full p-4 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-50" rows="3" placeholder="Timeline, location, quality..." />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-6 border-t border-slate-100">
                        <div className="space-y-3">
                            <h4 className="text-[10px] font-black text-blue-600 uppercase border-b border-blue-100 pb-1">Medical History</h4>
                            {['Allergy', 'Asthma', 'DM', 'HPN'].map(m => <CheckboxField key={m} label={m} />)}
                        </div>
                        <div className="space-y-3">
                            <h4 className="text-[10px] font-black text-blue-600 uppercase border-b border-blue-100 pb-1">Family History</h4>
                            {['Cancer', 'Diabetes', 'Heart Dis.'].map(f => <CheckboxField key={f} label={f} />)}
                        </div>
                        <div className="space-y-4">
                            <h4 className="text-[10px] font-black text-blue-600 uppercase border-b border-blue-100 pb-1">Personal</h4>
                            <InputField label="Smoking" placeholder="pack-years..." />
                            <InputField label="Alcohol" placeholder="frequency..." />
                        </div>
                    </div>
                </div>
            )}
            ...
            {currentStep === 2 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in fade-in text-left">
                    <div className="space-y-3">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase border-b border-slate-100 pb-1">HEENT</h4>
                        {['Headache', 'Dizziness', 'Visual Blurring'].map(s => <CheckboxField key={s} label={s} />)}
                    </div>
                    <div className="space-y-3">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase border-b border-slate-100 pb-1">Cardio</h4>
                        {['Chest Pain', 'Palpitations', 'Dyspnea'].map(s => <CheckboxField key={s} label={s} />)}
                    </div>
                    <div className="space-y-3">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase border-b border-slate-100 pb-1">General</h4>
                        {['Fever', 'Chills', 'Malaise'].map(s => <CheckboxField key={s} label={s} />)}
                    </div>
                </div>
            )}
                ...
                <div className="space-y-10 animate-in fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                        <div className="space-y-2">
                            <h4 className="text-[10px] font-black text-blue-600 uppercase mb-4 tracking-wider">General Survey</h4>
                            {['Normal', 'In Distress', 'Dehydrated'].map(g => <CheckboxField key={g} label={g} />)}
                        </div>
                        <div className="col-span-2 bg-slate-50 p-6 rounded-xl border border-slate-100">
                            <h4 className="text-[10px] font-black text-slate-500 uppercase mb-4">Other Findings</h4>
                            <textarea className="w-full bg-white border border-slate-200 rounded-lg p-3 text-sm outline-none focus:ring-2 focus:ring-blue-50" rows="5" />
                        </div>
                    </div>
                </div>
            )}
            ...
            {currentStep === 4 && (
                <div className="space-y-8 animate-in fade-in text-left">
                    <div className="bg-white border-2 border-dashed border-blue-100 p-6 rounded-2xl">
                        <label className="text-[11px] font-black text-blue-700 uppercase mb-3 block">Clinical Impression / Diagnosis</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-[11px] font-black text-slate-500 uppercase mb-3 block">Non-Pharm Management</label>
                        <textarea className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm outline-none" rows="4" placeholder="Advice, diet, restrictions..." />
                    </div>
                </div>
            )}
...
export default MedicineConsultDetails;