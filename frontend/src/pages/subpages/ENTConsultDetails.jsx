...
const ENTConsultDetails = ({ visitData, onSave }) => {
  ...
  const InputField = ({
    ...
    <div className="flex flex-col text-left">
      <label
        className={`text-[10px] font-bold uppercase mb-1 tracking-tight ${dark ? "text-slate-400" : "text-slate-400"}`}
      >
        {label}
      </label>
      <input
  ...
  const CheckboxField = ({ label, checked }) => (
    <label className="flex items-center gap-2 cursor-pointer group">
      <div
        className={`w-4 h-4 rounded border flex items-center justify-center transition ${checked ? "bg-blue-600 border-blue-600" : "border-slate-300 group-hover:border-blue-300"}`}
      >
    ...
    <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 md:p-10 relative animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        {steps.map((s, i) => (
          <React.Fragment key={s.id}>
            <div className="flex flex-col items-center gap-2 min-w-[60px]">
              ...
              <span
                className={`text-[9px] font-black uppercase tracking-tighter ${currentStep === s.id ? "text-blue-600" : "text-slate-400"}`}
              >
                {s.label}
              </span>
            </div>
            {i !== steps.length - 1 && (
              <div className="flex-1 h-px bg-slate-100 mx-1 mt-[-20px] min-w-[20px]" />
            )}
          </React.Fragment>
        ))}
      </div>
        ...
        <div className="space-y-8 animate-in fade-in text-left">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <textarea
                className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl text-sm italic text-slate-600"
                rows="2"
            ...
            <div>
              ...
              <textarea
                className="w-full p-4 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-50"
                rows="3"
                placeholder="Duration, onset, associated symptoms..."
              />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-6 border-t border-slate-100">
            <div className="space-y-3">
              <h4 className="text-[10px] font-black text-blue-600 uppercase border-b border-blue-100 pb-1">
                Specialty History
              </h4>
              {[
                "Hearing Loss",
            ...
            <div className="space-y-3">
              ...
              {["Tonsillectomy", "Septoplasty", "Myringotomy"].map((f) => (
      ...
      {currentStep === 2 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in fade-in text-left">
          <div className="space-y-3">
            <h4 className="text-[10px] font-black text-slate-400 uppercase border-b border-slate-100 pb-1">
              Ear
            </h4>
            {["Otalgia", "Otorrhea", "Vertigo", "Itchiness"].map((s) => (
          ...
          <div className="space-y-3">
            ...
            {["Congestion", "Epistaxis", "Post-nasal drip", "Anosmia"].map(
              (s) => (
                <CheckboxField key={s} label={s} />
              ),
            )}
          ...
          <div className="space-y-3">
            ...
            {["Dysphagia", "Hoarseness", "Odynophagia", "Lump in neck"].map(
              (s) => (
                <CheckboxField key={s} label={s} />
              ),
            )}
      ...
      {currentStep === 3 && (
        <div className="space-y-10 animate-in fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
            <div className="space-y-4">
              <h4 className="text-[10px] font-black text-blue-600 uppercase mb-4 tracking-wider">
                Otoscopy
              </h4>
            ...
            <div className="col-span-2 bg-slate-50 p-6 rounded-xl border border-slate-100">
              <h4 className="text-[10px] font-black text-slate-500 uppercase mb-4">
                Anterior Rhinoscopy / Pharyngoscopy
              </h4>
              <textarea
                className="w-full bg-white border border-slate-200 rounded-lg p-3 text-sm outline-none focus:ring-2 focus:ring-blue-50"
                rows="5"
                placeholder="Document turbinates, septum, tonsils, etc..."
              />
            </div>
          </div>
        </div>
      ...
      {currentStep === 4 && (
        <div className="space-y-8 animate-in fade-in text-left">
          <div className="bg-white border-2 border-dashed border-blue-100 p-6 rounded-2xl">
            <label className="text-[11px] font-black text-blue-700 uppercase mb-3 block">
              ENT Clinical Impression
            </label>
            <textarea
              className="w-full bg-white border border-blue-200 rounded-lg p-3 text-sm outline-none focus:ring-2 focus:ring-blue-100"
              rows="3"
            />
          </div>
          <div className="space-y-2">
            <label className="text-[11px] font-black text-slate-500 uppercase mb-3 block">
              Management Plan
            </label>
            <textarea
              className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm outline-none"
              rows="4"
              placeholder="Procedures, home care, follow-up..."
            />
          </div>
        </div>
      )}
      ...
      {currentStep === 6 && <AttachmentModule onFinalize={handleFinalize} />}
